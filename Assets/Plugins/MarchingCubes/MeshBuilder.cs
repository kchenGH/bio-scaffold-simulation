...
public class MeshBuilder : System.IDisposable
{
    ...
    {
        Debug.Log($"MeshBuilder created: size=({x},{y},{z}), budget={budget}");
        Initialize((x, y, z), budget, compute);
    }
    public MeshBuilder(Vector3Int dims, int budget, ComputeShader compute)
    {
        Debug.Log($"MeshBuilder created (vec3): size=({dims.x},{dims.y},{dims.z}), budget={budget}");
    ...
    {
        Debug.Log("BuildIsosurface() running...");
    ...
    public void Initialize((int, int, int) dims, int budget, ComputeShader compute)
    {
        _grids = dims;
    ...
    public void RunCompute(ComputeBuffer voxels, float target, float scale)
    {
        ...
        Debug.Log("Dispatching kernel 0...");
        _compute.DispatchThreads(0, _grids);
        Debug.Log("Compute shader dispatched (kernel 0)");
        ...
        Debug.Log("Dispatching kernel 1...");
        _compute.DispatchThreads(1, 1024, 1, 1);
        Debug.Log("Compute shader dispatched (kernel 1)");
